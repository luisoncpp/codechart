...
import { groupHeaderBoxId } from "./group-header-metrics";
...
export class ElkLayoutEngine implements LayoutEngine {
  ...
  async layout(graph: ProjectGraph, options?: LayoutOptions): Promise<LayoutedGraph> {
    ...
    const ignoredIds = new Set(graph.groups.map((g) => groupHeaderBoxId(g.id)));
    const result = await this.elk.layout(buildElkGraph(graph, options));
    return toLayoutedGraph(result, { groupIds, moduleIds, descriptionIds, ignoredIds });
  }
}
