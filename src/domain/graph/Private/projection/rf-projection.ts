...
import { groupNode, moduleNode } from "./rf-projection-nodes";
import { symbolsByModuleId } from "./rf-projection-module-symbols";
...
export function projectGraph(
  ...
  const ctx = withModuleSymbols(
    buildProjectionCtx(graph, options, layoutIndexes),
    layout,
    moduleById,
  );
  ...
  tintModuleGroupColors(moduleNodes, { groupById });
...
type ProjectionCtxBase = Omit<ProjectionCtx, "moduleSymbols">;
...
function withModuleSymbols(
  base: ProjectionCtxBase,
  layout: LayoutedGraph,
  moduleById: Map<string, ModuleNode>,
): ProjectionCtx {
  return {
    ...base,
    moduleSymbols: symbolsByModuleId(layout, moduleById, base),
  };
}

function tintModuleGroupColors(
  moduleNodes: ReturnType<typeof moduleNode>[],
  refs: { groupById: Map<string, GroupNode> },
) {
  const { groupById } = refs;
