...
8. `edge-style` / `EdgeLayer` render added edges **green** (full opacity), removed edges **red** with an **X** head instead of an arrow, and rename edges **yellow** (`#d97706`, arrow head) from the deleted module to the created one.
...
14. **Deleted module inspection & rename navigation** — selecting a deleted module (ghost card) opens its inspection details (`ModuleInspection`), showing its path, metadata, deleted status, and whether it was renamed. If renamed, a clickable button links to the target module, centering the viewport on that module via `store.focusOn(targetId)`.
