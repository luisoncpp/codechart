...
3. Existing valid body edits debounce for 400 ms and display auto-save status. The store permits one save at a time, coalesces later changes into the latest snapshot, and exposes Retry after a failure.
4. Resolve inline (via the card header) or Resolve in the sidebar removes a note immediately. Resolve all removes every note in the current sidebar filter as one mutation. **Copy all comments** copies every active Review Note as `path:line-range` plus body to the system clipboard; its button changes to Copied (or Copy failed) to confirm the result. One five-second Undo restores the latest single note or batch at its original array positions; a later resolution replaces the prior Undo opportunity.
