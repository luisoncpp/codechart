...
- Semantic zoom is a **pure projection over the immutable graph** (TDD §8): new `projectForZoom(graph, collapsedGroupIds)` (`domain/graph/Private/zoom-projection.ts`) drops modules + nested groups under a collapsed group, re-routes their edges onto the outermost collapsed ancestor box, drops self-loops and dedups (a merged violation survives). `topLevelGroupIds` = L0 default; `levelFromZoom(factor)` maps scroll zoom → 0/1/2 (`<0.55 / <1.7 / ≥1.7`). Pipeline: `projectForZoom` → `layout(reduced, sizeOpts)` → `projectGraph(reduced, layout, renderOpts)`.
