...
Geometry rebuilds (`writeGeometry`: `d` + crosses + arrows) run on graph/layout changes and on clip-cell change; pan within the same clip cell does not call `writeGeometry`.
