...
| `project_search` | `features/graph_canvas/Private/project_search/` | Nested deep module (public interface: `ProjectSearch`). The bar's open flag lives in `CanvasUiState` (`graph_canvas/Private/canvas-ui-state.ts`, exported via the facade) so the toolbar **Search ▾** menu (`SearchMenu`) can open it; query/result/active-index state remains component-local so typing never re-renders the canvas. Debounce 300 ms, min 2 chars, stale-response sequence guard. Navigation reuses `store.focusOn` only (select, expand ancestors, center). Match stepping lives in the shared `Private/match-stepper.ts` (also used by find-in-frame). |
