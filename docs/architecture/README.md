...
| [cli.md](./cli.md) | `codechart-cli` (`parse` / `groups` / `analyze` / `check` / `help`) | Debug dump vs CI diagnostic gate |
...
| [graph-canvas.md](./graph-canvas.md) | `domain/projection` (RF view model + diff-overlay stamping) + `features/graph_canvas` (React Flow) + `inspection_panel` + session selection/layout | `ProjectGraph` + layout → rendered map (sample aesthetic) + selection-driven inspection; also the **domain layering table** (graph → layout/diff → projection, one-way) |
...
| [help-menu.md](./help-menu.md) | `help_menu` | Toolbar Help dropdown menu and rendered Markdown documentation modals with Copy for LLMs |
