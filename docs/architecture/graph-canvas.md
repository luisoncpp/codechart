...
| `FacadeBypassList` | `project_loader/Private/FacadeBypassList.tsx` | When `architectureViolations(graph)` is non-empty, toolbar chip **1 architecture issue** / **N architecture issues** opens a modal with copyable messages (facade bypasses + deduped import cycles). |
...
- **Edge:** solid grey arrow (`import`); red + thicker when `isViolation` (facade bypass
  ...
  arrowheads; the merged `d` is **clipped** to an inflated visible rect (hysteresis clip cell — rebuild on
  cell change + geometry rebuild, not every pointermove); zoom ≥ 1.5 adds arrow geometry for non-cross
  segments; removed-diff **X** heads clip with the same visible segment set as strokes.
...
- **Heatmap overlay:** View ▾ **Heatmap** is always available (not git-gated). Radios: **Activity | Risk | Instability** (Activity default when git exists; enabling without git selects **Instability**). Activity/Risk stay disabled without git (tooltip: “Requires a git repository”). Activity/Risk read `ModuleMetrics.churn` / `bugRisk` stamped by `analyze_project` for the session lookback window (90 days default, Rust `git::enrich_module_metrics`); `GraphSessionStore` owns the day count and `HeatmapLegend` reanalyzes on change. **Instability** is Martin `Ce / (Ce + Ca)` from unique solid-`import` neighbors (`instabilityRatio` via `importsOf` / `importedBy`) — not `ModuleMetrics` fields, not soft edges; isolated modules (Ce+Ca = 0) score 0. While the heatmap is on, `HeatmapLegend` renders at the canvas top-right below the level badge (git modes: “Last N days”; Instability: that label). `computeHeatProjection` (`heat-scores.ts`,
...
- **Group descriptions (multi-level):** `rf-projection` threads both `descriptionShort` and
  ...
  - **L1 (expanded):** `GroupDescription` draws `descriptionShort` **directly in the group** (no box) at
    ...
    same scale as the module filenames, not counter-scaled. `GroupDescription` parses inline Markdown
    so bold, code, and links render styled HTML. **Hovering the short text opens a custom
    tooltip with the full `descriptionLong`** (`GroupDescription.tsx` + `DescriptionTooltip.tsx`): a
    `position: fixed` div portaled to `document.body`, rendering rich Markdown (paragraphs, lists, code,
    headings, wiki-links) in screen space (unaffected by canvas zoom, never clipped by the node) with
    no OS/native-`title` length truncation; it scrolls internally past 60vh and flips above the cursor
    in the bottom half of the viewport. The `<p>` takes
