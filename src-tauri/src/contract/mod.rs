...
mod contract_tests {
    ...
    fn golden_matches_the_planted_fixture_facts() {
        ...
        assert_eq!(
            graph.groups.len(),
            5,
            "3 responsibility + 1 aggregate + 1 cross-cutting"
        );
        assert_eq!(graph.modules.len(), 13);
        assert_eq!(
            graph.edges.len(),
            22,
            "20 imports + 1 soft (event) edge + 1 soft (interface seam) edge"
        );
        assert_eq!(
            graph.diagnostics.len(),
            2,
            "unresolved import + facade bypass"
        );
        ...
        let bypass = graph
            .edges
            .iter()
            .find(|e| e.source == "src/ui/TodoList.tsx" && e.target == "src/core/store.ts");
        assert!(
            bypass
                .expect("planted facade-bypass import is present")
                .is_violation
        );
        ...
        let doc = core
            .annotation
            .as_ref()
            .expect("group doc from core.group.md");
        assert!(
            doc.description_long.is_some(),
            "group body populates annotation"
        );
        assert_eq!(
            core.parent_id.as_deref(),
            Some("app"),
            "nested under the app group"
        );
    ...
    fn builder_accepts_the_golden_model() {
        ...
        builder
            .build()
            .expect("golden model satisfies every §2.2 invariant");
    }
}
