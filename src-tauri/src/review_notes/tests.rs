...
fn note(path: &str, lines: &[&str]) -> ReviewNote {
    ReviewNote {
        id: "note-1".to_string(), path: path.to_string(), start_line: 1,
        end_line: lines.len(), anchor_lines: lines.iter().map(|line| line.to_string()).collect(),
...
fn save(root: &Path, note: ReviewNote) {
    save_review_notes(root.to_str().unwrap(), ReviewNotesDocument { version: 1, notes: vec![note] }).unwrap();
}
...
fn keeps_stationary_anchor_and_round_trips_atomically() {
    ...
    let loaded = load_review_notes(root.path().to_str().unwrap(), vec!["src/a.ts".to_string()]).unwrap();
...
fn moves_exact_and_normalized_same_file_anchors() {
    ...
    save(root.path(), note("src/a.ts", &["const x = 1;", "return x;"]));
    write(root.path(), "src/a.ts", "before\ninserted\nconst x = 1;\nreturn x;\n");
    let exact = load_review_notes(root.path().to_str().unwrap(), vec!["src/a.ts".to_string()]).unwrap();
    assert_eq!(exact.notes[0].start_line, 3);
    write(root.path(), "src/a.ts", "before\ninserted\nconst   x=1;\nreturn   x ;\n");
    let normalized = load_review_notes(root.path().to_str().unwrap(), vec!["src/a.ts".to_string()]).unwrap();
    assert_eq!(normalized.notes[0].anchor_lines, vec!["const   x=1;", "return   x ;"]);
}
...
fn moves_renamed_file_but_drops_ambiguous_or_unsupported_existing_anchor() {
    ...
    let moved = load_review_notes(root.path().to_str().unwrap(), vec!["src/new.ts".to_string()]).unwrap();
    assert_eq!(moved.notes[0].path, "src/new.ts");
    save(root.path(), note("src/new.ts", &["anchor"]));
    let deleted = load_review_notes(root.path().to_str().unwrap(), vec!["src/other.ts".to_string()]).unwrap();
    ...
    let ambiguous = load_review_notes(root.path().to_str().unwrap(), vec!["src/a.ts".to_string(), "src/b.ts".to_string()]).unwrap();
...
fn preserves_malformed_file_and_rejects_unsafe_document() {
    ...
    assert!(save_review_notes(root.path().to_str().unwrap(), ReviewNotesDocument { version: 1, notes: vec![unsafe_note] }).is_err());
}
...
fn reconciles_overlapping_notes_independently() {
    ...
    second.start_line = 2;
    second.end_line = 3;
    save_review_notes(root.path().to_str().unwrap(), ReviewNotesDocument { version: 1, notes: vec![note("src/a.ts", &["a", "b"]), second] }).unwrap();
    let loaded = load_review_notes(root.path().to_str().unwrap(), vec!["src/a.ts".to_string()]).unwrap();
