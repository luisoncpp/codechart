...
impl ReviewNotesDocument {
    fn empty() -> Self {
        Self { version: VERSION, notes: Vec::new() }
    }
}

pub fn load_review_notes(root: &str, module_paths: Vec<String>) -> Result<ReviewNotesDocument, String> {
...
fn find_anchor(root: &str, note: &ReviewNote, paths: &[String]) -> Result<Option<(String, usize, Vec<String>)>, String> {
    if paths.len() == 1 {
    ...
    Ok(if normalized.len() == 1 { normalized.into_iter().next() } else { None })
}

fn matches_for(root: &str, paths: &[String], anchor: &[String], normalized: bool) -> Result<Vec<(String, usize, Vec<String>)>, String> {
    let mut matches = Vec::new();
    for path in paths {
        let lines = read_lines(root, path)?;
        if lines.len() < anchor.len() { continue; }
        for start in 0..=lines.len() - anchor.len() {
            let actual = &lines[start..start + anchor.len()];
            let equal = if normalized {
                actual.iter().map(normalize).eq(anchor.iter().map(normalize))
            } else { actual == anchor };
            if equal { matches.push((path.clone(), start, actual.to_vec())); }
        }
    }
    Ok(matches)
}
...
fn read_lines(root: &str, path: &str) -> Result<Vec<String>, String> {
    Ok(std::fs::read_to_string(project_file(root, path)).map_err(display_error)?
...
fn validate_document(document: &ReviewNotesDocument) -> Result<(), String> {
    if document.version != VERSION { return Err(format!("unsupported review notes version: {}", document.version)); }
    let mut ids = HashSet::new();
    for note in &document.notes {
        if note.id.trim().is_empty() || !ids.insert(&note.id) { return Err("review note ids must be unique and nonblank".to_string()); }
        if note.body.trim().is_empty() { return Err("review note body cannot be blank".to_string()); }
        if note.start_line == 0 || note.end_line < note.start_line { return Err("review note line range is invalid".to_string()); }
        if note.anchor_lines.len() != note.end_line - note.start_line + 1 { return Err("review note anchor length does not match its range".to_string()); }
...
fn validate_relative_path(path: &str) -> Result<(), String> {
    if path.is_empty() || path.contains('\\') { return Err("review note path must be a relative POSIX path".to_string()); }
    ...
    if path.split('/').any(|part| part.is_empty() || part == ".") { return Err("review note path must be normalized".to_string()); }
    Ok(())
}

fn project_file(root: &str, path: &str) -> PathBuf { Path::new(root).join(path) }
fn notes_path(root: &str) -> PathBuf { Path::new(root).join(NOTES_DIR).join(NOTES_FILE) }
fn display_error(error: std::io::Error) -> String { error.to_string() }

fn write_document(root: &str, document: &ReviewNotesDocument) -> Result<(), String> {
    let path = notes_path(root);
    let parent = path.parent().ok_or_else(|| "review notes path has no parent".to_string())?;
